...
def main():
    ...
    #os.system("sudo mkdir "+DATA_DIR)
    #os.system("sudo mkdir "+DATASET_PATH)
    mkdir(DATA_DIR)
    mkdir(DATASET_PATH)
...
def download(filename, url):
    if os.path.exists(filename):
        ...
        # TODO: security lol
        os.system('sudo wget -nc {} -O {}'.format(url, filename))
        if filename.endswith('.tgz') or filename.endswith('.tar.gz'):
            os.system('sudo ls *gz | sudo xargs -n 1 tar xzvf')
        elif filename.endswith('.zip'):
            os.system('sudo unzip *.zip')
