...
def save_set(fold, x, y, suffix='png'):
    examples = []
    fp = open('mnist32_{}.dataset'.format(fold), 'w')
    print("Writing MNIST32 dataset {}".format(fold))
    for i in tqdm(range(len(x))):
        label = y[i]
        img_filename = 'mnist32/{}/{:05d}_{:d}.{}'.format(fold, i, label, suffix)
        img_filename = os.path.join(DATA_DIR, img_filename)
        if not os.path.exists(img_filename):
            Image.fromarray(x[i]).resize(size=(32, 32)).save(os.path.expanduser(img_filename))
        entry = {
...
def download_mnist(latest_md5):
    ...
    with open('mnist32-05.dataset', 'w') as fp:
        for example in train + test + val:
            if int(example['label']) < 6:
                fp.write(json.dumps(example) + '\n')
    with open('mnist32-69.dataset', 'w') as fp:
        for example in train + test + val:
            if int(example['label']) >= 6:
                fp.write(json.dumps(example) + '\n')
    ...
    for class_idx in range(10):
        fp_set = open('mnist32-{}.dataset'.format(class_idx), 'w')
        fp_openset = open('mnist32-not{}.dataset'.format(class_idx), 'w')
        for example in train + test + val:
    ...
    for class_idx in [2, 3, 4, 5, 6, 7]:
        fp_set = open('mnist32-0{}.dataset'.format(class_idx), 'w')
        fp_openset = open('mnist32-{}9.dataset'.format(class_idx), 'w')
        for example in train + test + val:
            if int(example['label']) <= class_idx:
                fp_set.write(json.dumps(example) + '\n')
            else:
