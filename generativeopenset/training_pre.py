...
def train_wgan(networks, optimizers, dataloader, epoch=None, **options):
    ...
    for i, (images, class_labels) in enumerate(dataloader):
        ...
        for d_iter in range(critic_iter):
            ...
            # 노이즈 사이즈:torch.Size([64, 1024]), sample_scale size:4, latent_size:64
            fake_images = netG(noise, sample_scale)
            logits = netD(fake_images)[:,0]
            loss_fake_sampled = logits.mean()
            log.collect('Discriminator Sampled', loss_fake_sampled)
            loss_fake_sampled.backward(one)
            ...
            # d_loss = loss_fake_sampled - d_loss_real + gp
        ...
        errSampled = logits.mean()
        errSampled.backward(mone)
        ...
        if i % 100 == 1:
            ...
            demo_wgan(networks, images, fixed_noise, ac_scale, sample_scale, result_dir, epoch, i)
    return True
...
def train_gan(networks, optimizers, dataloader, epoch=None, **options):
    ...
    for i, (images, class_labels) in enumerate(dataloader):
        images = Variable(images) # float 0~1
        labels = Variable(class_labels) # correct class: 1 / uncorrect class: -1
...
def demo_wgan(networks, images, fixed_noise, ac_scale, sample_scale, result_dir, epoch=0, idx=0):
    # netE = networks['encoder']
    netG = networks['generator']
    ...
    aac_after = netG(demo_fakes, ac_scale)
