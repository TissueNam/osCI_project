...
#EMNIST_LOCATION = '/home/kcm/data/mnist.dataset'
EMNIST_LOCATION = '/home/kcm/data/emnist.dataset'
...
def main():
    ...
    # 추가함
    print('기존 테스트 데이터셋')
    test_classifier(classifier, load_testing_dataset())
    test_classifier(augmented_classifier, load_testing_dataset())
    
    # open set?
    print('open set')
    test_classifier(classifier, load_open_set())
    test_classifier(augmented_classifier, load_open_set())    
...
def load_training_dataset():
    ...
    def generator():
        for images, labels in dataloader:
            #images의 크기: torch.Size([64, 1, 28, 28])
...
def load_open_set():
    def generator():
        for images, labels in CustomDataloader(EMNIST_LOCATION, fold='test', image_size=28, shuffle=False):
            labels[:] = NUM_CLASSES
            new_type_images = images.mean(dim=1).unsqueeze(1)
            yield images.mean(dim=1).unsqueeze(1), labels
    return generator()
...
def train_generative_model(encoder, generator, discriminator, dataset):
    ...
    for images, labels in dataset:
        ...
        #print('Autoencoder loss: {:.03f}, Generator loss: {:.03f}, Disc. loss: {:.03f}'.format(mse_loss, gen_loss, disc_loss))
        gen_adam.step()
